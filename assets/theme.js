...
(function(){
  ...
  theme.cart = {
    ...
    _updateCart: function(params) {
      ...
      .then(response => { 
        ...
        // 🔁 update shipping bar AFTER cart count changes
        if (theme.ShippingBar && typeof theme.ShippingBar.update === 'function') {
  ...
  theme.CartForm = (function() {
    ...
    CartForm.prototype = Object.assign({}, CartForm.prototype, {
      ...
      ==============================================================================*/
      initRecAtcButtons: function() {
        var _this = this;

        // Remove old listeners by cloning if needed (optional, safe approach)
        var buttons = this.form.querySelectorAll(selectors.recAtc);
        buttons.forEach(function(btn) {
          // Avoid attaching duplicate listeners if you prefer
          if (btn.dataset.recAtcBound === 'true') return;
          btn.dataset.recAtcBound = 'true';

          btn.addEventListener('click', function(evt) {
            _this.onRecAtcClick(evt.currentTarget);
          });
        });
      },

      onRecAtcClick: function(button) {
        var variantId = button.getAttribute('data-variant-id');
        if (!variantId) return;

        button.classList.add(classes.btnLoading);
        ...
        fetch((window.Shopify && window.Shopify.routes && window.Shopify.routes.root ? window.Shopify.routes.root : '/') + 'cart/add.js', {
          ...
          .then(function(data) {
            button.classList.remove(classes.btnLoading);

            // Handle error from /cart/add.js
            if (data && data.status === 422 && data.description) {
              alert(data.description);
              return;
            }

            // Rebuild cart using your existing flow
            document.dispatchEvent(new CustomEvent('cart:build'));

            // Optionally: update bubble directly if you want extra safety
            // theme.cart.getCart && theme.cart.getCart().then(function(cart) {
            //   document.dispatchEvent(new CustomEvent('cart:updated', { detail: { cart: cart } }));
            // });
          })
          .catch(function(err) {
            console.error('Recommendation ATC error', err);
            button.classList.remove(classes.btnLoading);
          });
      },
  ...
  theme.Drawers = (function() {
    ...
    Drawers.prototype = Object.assign({}, Drawers.prototype, {
      ...
      close: function(evt) {
        ...
        //   insideDrawer: evt && evt.target && evt.target.closest('.drawer')
        // });
    ...
    function productMouseover(evt) {
      ...
      // No quick view on mobile breakpoint
       //if (!theme.config.bpSmall) { 
        el.removeEventListener('click', productMouseover);
  ...
  theme.preloadProductModal = function(handle, productId, btn) {
    ...
    .then(text => {
      ...
      var modalm = '#QuickShopModal-' + productId;
      
      // theme.sections.register('product', theme.Product, holder);
      
      //  $('#QuickShopModal-' + productId).addClass('modal--is-active');
      // console.log(modalId + 'data' + name);
  ...
  class PredictiveSearch extends HTMLElement {
    constructor() {
      ...
      this.closeBtn.addEventListener('click', e => { e.preventDefault(); this.close2(); });
  ...
  theme.announcementBar = (function() {
    var args = {
      autoPlay: 5000,
      avoidReflow: true,adaptiveHeight: true,
    ...
    Collection.prototype = Object.assign({}, Collection.prototype, {
      ...
      initFilters: function() {
        ...
        $('.accordion__header').click(function(e) {
                $(this).parent('.collection-sidebar__group').find('.accordion__body').slideToggle(400);                 
                $('.accordion__body').not($(this).parent('.collection-sidebar__group').find('.accordion__body')).hide();                
                e.preventDefault();
                e.stopPropagation();
        }); 
        ...
        detailsEl.forEach((detail) => {
          ...
          detail.addEventListener('mouseover', () => {
            if (!detail.hasAttribute('open')) {
              ...
              $('.custom-menu-slides').slick('refresh');
            }
          });
  ...
  theme.Product = (function() {
    ...
    Product.prototype = Object.assign({}, Product.prototype, {
      ...
      toggleInventoryQuantity: function(variant, variantInventoryObject) {
        const { policy } = variantInventoryObject || {};
        ...
        var requiredTag = 'Eclipse_Info'; 
        
        //alert(parseInt(quantity));alert(parseInt(this.settings.inventoryThreshold));
        
        if (parseInt(quantity) <= parseInt(this.settings.inventoryThreshold) && variant.inventory_policy === 'continue') {
          console.log('outofstock');
          el.parentNode.classList.add('inventory--low');

          if (productTagss.includes(requiredTag)) {
            console.log('Product has the tag: ' + requiredTag);
            if (quantity >= 1) {
              el.parentNode.classList.remove('inventory--low');
              el.textContent = theme.strings.inStockLabel;
            } else {
              el.textContent = theme.strings.titaniumStockLabel;            
            }
          } else {
            console.log('Product does not have the tag: ' + requiredTag);
            if (quantity >= 1) {
              el.parentNode.classList.remove('inventory--low');
              el.textContent = theme.strings.inStockLabel;
            } else {
              el.textContent = theme.strings.oneStockLabel;            
            }
          }          
          
        } else {console.log('instock');
          el.parentNode.classList.remove('inventory--low');
          el.textContent = theme.strings.inStockLabel;
        }
  ...
  //   test: "hello world",
  //   update: function(){
  //   console.log("Updated Shipping Bar");
  //   };
  // };
  ...
  //   // var message_bar = document.querySelector('.promo__message_field'); 
  //   // var msgbr_promotetext = message_bar.dataset.promote;
  //   // var giftmsgthreshold = message_bar.dataset.threshold;
    
  //   var progressbar = document.querySelector('.shipping-bar'); 
    
  //   function update()
  //   {
  //     $.getJSON('/cart.js').then(
  //       function(cart) { 
  //         var val_left = threshold - cart.total_price;
  //         var val_left_money = theme.Currency.formatMoney(val_left, theme.settings.moneyFormat);
  //         if(val_left <= 0){
  //           bar.innerHTML = '<span class="shipping-bar__text text--small unlockedjs unlocked">'+unlocked_txt+'</span>';
  //         } else {
  //           bar.innerHTML = '<span class="shipping-bar__text text--small promojs">' + promote_txt.replace('[value]',val_left_money) + '</span>';
  //         }
  //         progressbar.style.setProperty("--progress", Math.min(parseFloat(cart.total_price) / threshold, 1));

  //         // var giftval_left = giftmsgthreshold - cart.total_price;
  //         // var giftval_left_money = theme.Currency.formatMoney(giftval_left, theme.settings.moneyFormat);
          
  //         // if(cart.total_price > 50000){
  //         //   message_bar.innerHTML = '<small>Your order qualifies for a complimentary Lash Single Wrap Bracelet. It will be added to your shipment automatically - no action needed.</small>';
  //         // } else {
  //         //   message_bar.innerHTML = '<small>' + msgbr_promotetext.replace('[value]',giftval_left_money) + '</small>';
  //         // }
  //       }      
  //     );
  //   }
...
theme.ShippingBar = (function () {
  ...
  var promote_txt  = bar.dataset.promote || '';
  var unlocked_txt = bar.dataset.unlocked || '';
  var threshold    = parseInt(bar.dataset.threshold, 10) || 0;

  function update() {
    return $.getJSON('/cart.js').then(function (cart) {

      if (!cart || !cart.total_price) return;

      var val_left = threshold - cart.total_price;
      var val_left_money = theme.Currency.formatMoney(
        Math.max(val_left, 0),
        theme.settings.moneyFormat
      );
      ...
      if (val_left <= 0) {
        bar.innerHTML =
          '<span class="shipping-bar__text text--small unlockedjs unlocked">' +
          unlocked_txt +
          '</span>';
      } else {
        bar.innerHTML =
          '<span class="shipping-bar__text text--small promojs">' +
      ...
      progressbar.style.setProperty(
        '--progress',
        Math.min(cart.total_price / threshold, 1)
      );
    });
  }
  ...
  return { update: update };
      ...
      $('.md-sticky-prd .add-to-cart').click(function(e){
      e.preventDefault();
        var selval =  $('.select_pop').val();
        if (selval !== null && selval !== "") {
          ...
          $('.show_error_msg').addClass('active');
              setTimeout(function() {
                        $('.show_error_msg').removeClass('active');
              }, 3000);
        }
    });
})();
...
//   $('.p-carousel').slick('unslick');
// } else {  
  $('.p-carousel').flickity({
  cellAlign: 'left',
  contain: true
});
// $('.coll-carousel').flickity({
//   cellAlign: 'left',
//   contain: true,pageDots: false,wrapAround: true
// });
 
//}
// $(window).resize(function(){
//   var dtrow = $('.p-carousel').data('perrow');
//     var viewportWidth = jQuery(window).width();
//     if (viewportWidth < 769) {
//         $('.p-carousel').slick('unslick');
//     } else {
...
$('.similar_p_v .variant-wrapper span').click(function(){
  $(this).slideToggle();
  $(this).parent('.variant-wrapper').find('ul').slideToggle();
  $(this).parent('.variant-wrapper').find('.closebtn').slideToggle();
});
$('.similar_p_v .variant-wrapper .closebtn').click(function(){
  $(this).slideToggle();
  $(this).parent('.variant-wrapper').find('ul').slideToggle();
  $(this).parent('.variant-wrapper').find('span').slideToggle();
});
$('.variant-wrapper ul a').on("click", function () {
  var dtimg = $(this).data('img');
  var dtmeta = $(this).data('title');
  var dtprice = $(this).data('price');
  var dturl = $(this).data('href');console.log(dturl);
  $(this).parent().parent('ul').find('li').removeClass('activeprod');
  var datasrcset = dtimg +'&width=360 360w,' + dtimg + '&width=540 540w,' + dtimg + '&width=720 720w,'+ dtimg + '&width=900 900w,'+ dtimg + '&width=1080 1080w,';
  $(this).parent('li').addClass('activeprod');
  $(this).parent().parent().parent().parent().parent().parent().parent().parent('.grid-product').find('.grid-product__image-mask img').attr("src", dtimg);
  $(this).parent().parent().parent().parent().parent().parent().parent().parent('.grid-product').find('.grid-product__image-mask img').attr("srcset", datasrcset);

  $(this).parent().parent().parent().parent().parent().parent().parent().parent('.grid-product').find('.grid-product__title small').text(dtmeta);
  $(this).parent().parent().parent().parent().parent().parent().parent().parent('.grid-product').find('.grid-product__link').attr("href", dturl);
  $(this).parent().parent().parent().parent().parent().parent().parent().parent('.grid-product').find('span.price-item--regular').text(dtprice);
});
$(document).ready(function(){
  ...
  $(document).on('click', '.size-chart-link', function(e){
    e.preventDefault();
    $('.inch').toggleClass('d-none');
    $('.centimeter').toggleClass('d-none');
  }); 

  
   var s = $(".hdrpartcsssticky");
	var pos = s.position();
	...
	$(window).scroll(function() {
      ...
      //alert(windowpos);
		if (windowpos >= pos.top) {
			s.addClass("stickycss");
            $('.template-index #HeaderWrapper, .template-collection-withbanner #HeaderWrapper').removeClass('is-light');
            $('.template-index #HeaderWrapper, .template-collection-withbanner #HeaderWrapper').addClass('is-dark');
		} else {
			s.removeClass("stickycss");	
            $('.template-index #HeaderWrapper, .template-collection-withbanner #HeaderWrapper').addClass('is-light');
            $('.template-index #HeaderWrapper, .template-collection-withbanner #HeaderWrapper').removeClass('is-dark');
		}
	});
  ...
  //     $(".quick-product__btn").removeClass("quick-product__btn--not-ready");
  //     $('.quick-product__btn').attr("aria-expanded", "false");
  // }
});
$(document).ready(function() {
  if ($(window).width() < 769) {
      $('.quick-product__btn').trigger('click');
  }

  $(".site-nav__dropdown-link_hoveredimags").on("mouseenter", function() {
    var imgsrc = $(this).attr('data-image');
    var colltitle = $(this).attr('data-title');
    var collurl = $(this).attr('data-url');
    if($(this).attr('data-image') != ''){
      $(this).parent().parent().parent('.grid').find('.subcatmenus__collimage img').attr('src', imgsrc);
      $(this).parent().parent().parent('.grid').find('.subcatmenus__collimage .rte h3').text(colltitle);
      $(this).parent().parent().parent('.grid').find('.subcatmenus__collimage a').attr('href', collurl);
    }
  });

  $(".product-single__meta .similar_p_v.singleoptionallvariants li a").on("mouseenter", function() {
    var title = $(this).attr('data-subtitle');
    $('.product-single__meta .product-single__title_withsubtitle small').text(title);
  });

  $(".product-single__meta .similar_p_v.singleoptionallvariants li a").on("mouseleave", function() {
    var title2 = $(this).parent().parent('ul').find('li.activvariant a').attr('data-subtitle');
    $('.product-single__meta .product-single__title_withsubtitle small').text(title2);    
  });

});


$('.collapse_label_1').click(function(){
 $(this).addClass('is-open');
  $('.drawer__fixed-header').addClass('hide');
  $(this).parent().parent('.mobile-nav__item').find('.mobile_sub_2').addClass('is-open');
});

$('button.collapse_label_2').click(function(){
 $(this).toggleClass('is-open');
  $(this).parents().eq(1).find('.mobile_sub_3').toggleClass('is-open');
   // $('.mobile_sub_2').addClass('hide_Scroll'); 
  // $('.mobile_sub_2').scrollTop(0);
  // $('.mobile_sub_3').scrollTop(0);
});

$('.back_lbl_1').click(function(){
  $('.mobile_sub_2').removeClass('is-open');
  $('.drawer__fixed-header').removeClass('hide');
});
$('.back_lbl_2').click(function(){
  $('.mobile_sub_3').removeClass('is-open');
  $('.mobile_sub_2').removeClass('hide_Scroll');
});
...
$('.dot_products').click(function(){
    $(this).find('.dot-product').toggle();
});
...
$(window).on('resize', function(){
  if ($(window).width() < 960) {
      $('.mobile_slider').flickity({  cellAlign: 'left',  contain: true });
  }
});
...
//   cellAlign: 'left',fade: true, pageDots: false,
//   contain: true, prevNextButtons: false, autoPlay: 5000 
// });
...
$(".box-video").click(function(){
  ...
  $(this).addClass('open');
    ...
    $('.toolbar__item').css('display', 'block');

    
    $('.toolbar__item').appendTo('.custom-currency-append');
   
$(document).ready(function () {
    // Toggle the side drawer on side-bar-toggle click
    $('.side-bar-toggle').click(function (event) {
        event.stopPropagation(); // Prevent propagation to document

        // Toggle the open class on the side drawer
        $('.custom-side-box').addClass('open');
    });

    // Close the side drawer when clicking outside
    $(document).on('click', function (event) {
        // Check if the click happened outside the `.custom-side-box` and `.side-bar-toggle`
        if (!$(event.target).closest('.custom-side-box, .side-bar-toggle').length) {
            $('.custom-side-box').removeClass('open');
        }
    });
});
    ...
    $('.custom-side-box').removeClass('open'); // Close the side drawer
    //  $('.custom-side-box').css('display', 'none');
    });

$(window).on('resize', function () {
  $('.custom-menu-slides').slick('setPosition');
});

// var $carousel = $('.custom-menu-slides').flickity({
//   imagesLoaded: true,
//   contain: true,
...
// });
  $( document ).ready(function() {
 $('.custom-menu-slides').slick({
  arrows: true,
  infinite: true,
  speed: 300,
  slidesToShow: 4,
  slidesToScroll: 4,
  prevArrow: '<button type="button" class="slick-prev"> <svg width="10" height="20" viewBox="0 0 10 20" fill="none" xmlns="http://www.w3.org/2000/svg"> <path d="M9.75595 1.0774C10.0813 1.40278 10.0814 1.93047 9.75587 2.25589L2.01183 9.99979L9.75595 17.744C10.0813 18.0694 10.0814 18.5971 9.75587 18.9225C9.43042 19.248 8.9028 19.248 8.57739 18.9225L0.244062 10.589C0.0877991 10.4327 0 10.2208 0 9.99979C0 9.77878 0.0878353 9.56681 0.244099 9.41058L8.57742 1.07747C8.90284 0.751945 9.43049 0.751944 9.75595 1.0774Z" fill="#090909"/> </svg> </button>',
  nextArrow: '<button type="button" class="slick-next"> <svg width="10" height="20" viewBox="0 0 10 20" fill="none" xmlns="http://www.w3.org/2000/svg"> <path d="M0.244054 1.07752C-0.0813273 1.4029 -0.0813989 1.93059 0.244125 2.25601L7.98817 9.99991L0.244054 17.7442C-0.0813273 18.0695 -0.0813989 18.5972 0.244125 18.9227C0.569578 19.2481 1.0972 19.2481 1.42261 18.9227L9.75594 10.5891C9.9122 10.4329 10 10.2209 10 9.99991C10 9.77891 9.91216 9.56693 9.7559 9.4107L1.42258 1.07759C1.09716 0.752068 0.569506 0.752066 0.244054 1.07752Z" fill="black"/> </svg> </button>',
  responsive: [
    {
      breakpoint: 1024,
      settings: {
...
if (window.matchMedia("(max-width: 767px)").matches) {
  ...
  responsive: [
    ...
    $('.article_carousel.past_events').slick({
      autoplay: true,
        dots: false,
        arrows: true,
        infinite: false,
        slidesToShow: 3,
        slidesToScroll: 1, prevArrow: '<button type="button" class="slick-prev"> <svg width="10" height="20" viewBox="0 0 10 20" fill="none" xmlns="http://www.w3.org/2000/svg"> <path d="M9.75595 1.0774C10.0813 1.40278 10.0814 1.93047 9.75587 2.25589L2.01183 9.99979L9.75595 17.744C10.0813 18.0694 10.0814 18.5971 9.75587 18.9225C9.43042 19.248 8.9028 19.248 8.57739 18.9225L0.244062 10.589C0.0877991 10.4327 0 10.2208 0 9.99979C0 9.77878 0.0878353 9.56681 0.244099 9.41058L8.57742 1.07747C8.90284 0.751945 9.43049 0.751944 9.75595 1.0774Z" fill="#090909"/> </svg> </button>',
  nextArrow: '<button type="button" class="slick-next"> <svg width="10" height="20" viewBox="0 0 10 20" fill="none" xmlns="http://www.w3.org/2000/svg"> <path d="M0.244054 1.07752C-0.0813273 1.4029 -0.0813989 1.93059 0.244125 2.25601L7.98817 9.99991L0.244054 17.7442C-0.0813273 18.0695 -0.0813989 18.5972 0.244125 18.9227C0.569578 19.2481 1.0972 19.2481 1.42261 18.9227L9.75594 10.5891C9.9122 10.4329 10 10.2209 10 9.99991C10 9.77891 9.91216 9.56693 9.7559 9.4107L1.42258 1.07759C1.09716 0.752068 0.569506 0.752066 0.244054 1.07752Z" fill="black"/> </svg> </button>',
        responsive: [{
            breakpoint: 320,
            settings: {
  ...
  //$('.custom-menu-slides button.slick-next.slick-arrow').click();
  $('.custom-menu-slides').slick('refresh');

  });

  $('.innerPromoSlider').slick({
      autoplay: false,
        dots: false,
        arrows: true,
        infinite: true,
  ...
  nextArrow: '<button type="button" class="slick-next"> <svg width="10" height="20" viewBox="0 0 10 20" fill="none" xmlns="http://www.w3.org/2000/svg"> <path d="M0.244054 1.07752C-0.0813273 1.4029 -0.0813989 1.93059 0.244125 2.25601L7.98817 9.99991L0.244054 17.7442C-0.0813273 18.0695 -0.0813989 18.5972 0.244125 18.9227C0.569578 19.2481 1.0972 19.2481 1.42261 18.9227L9.75594 10.5891C9.9122 10.4329 10 10.2209 10 9.99991C10 9.77891 9.91216 9.56693 9.7559 9.4107L1.42258 1.07759C1.09716 0.752068 0.569506 0.752066 0.244054 1.07752Z" fill="black"/> </svg> </button>',
    ...
    // $('li.site-nav__item.site-nav__expanded-item.site-nav--is-megamenu').hover(
    //         function () {
    //             // On hover, add the class
    //             $('body').addClass('hover-active');
    //         },
    //         function () {
    //             // On hover out, remove the class
    //             $('body').removeClass('hover-active');
    //         }
    //     );

// footer toggle functionality
      $(document).on('click', '.custom-button-trigger', function() {
          // Remove 'is-open' class and reset height for all other custom-content-box elements
          $('.custom-button-trigger').not(this).removeClass('is-open');
          $('.custom-content-box').not($(this).next('.custom-content-box')).removeClass('is-open');
         $('.custom-content-box').not($(this).next('.custom-content-box')).css('height', 'revert-layer');
          
          // Toggle the clicked button and its associated content box
        //  $(this).toggleClass('is-open');
          let contentBox = $(this).next('.custom-content-box');
          if ($(this).hasClass('is-open')) {
          contentBox.css('height', 'auto'); // Set to 'auto' for the clicked button's content box
          } else {
          contentBox.css('height', '0'); // Collapse if toggled off
          }
       });
// footer toggle functionality
   
    $('ul.tabscoll').each(function(){
      var active, content, links = $(this).find('a');
      active = links.first().addClass('active');
      content = $(active.attr('href'));
      links.not(':first').each(function () {
...
// $('.site-nav__item.site-nav__expanded-item.site-nav--has-dropdown .site-nav__details').hover(function () {
//     $(this).toggleClass('selected').siblings().removeClass('selected');
// }); 

$(document).ready(function() {
  // const $target = $('.product-single__sticky:first-child');
  // const originalContent = $target.html(); // Store original content
  // // Handle mouseover on any preview image
  // $(document).on('mouseover', '.swatch_preview-img', function() {
  //   const newSrc = $(this).attr('src');
  //   if ($target.find('img').length > 0) {
  //     // If an image already exists, change its src
  //     $target.find('img').attr('src', newSrc);
  //   } else {
  //     // Else, append a new image
  //     $target.html(`<img src="${newSrc}" class="product-image">`);
  //   }
  // });

  // // Restore original on mouseout
  // $(document).on('mouseout', '.swatch_preview-img', function() {
  //   // Restore original content
  //   const $video = $target.find('video');
  //   if ($video.length) {
  //     const videoClone = $video.clone(); // Clone the video element
  //     // Ensure autoplay and muted for browser compatibility
  //     videoClone.attr({
  //       autoplay: true,
  //       muted: true,
  //       loop: true,
  //       playsinline: true
  //     });
  //     $target.html(videoClone); // Replace target with the video
  //     const videoEl = $target.find('video').get(0);
  //     if (videoEl) {
 ...
 const $target2 = $('.prodslide__0');
  const originalContent2 = $target2.html(); // Store original content

  // Handle mouseover on any preview image
  $(document).on('mouseover', '.swatch_preview-img', function() {
    const newSrc2 = $(this).attr('src');

    if ($target2.find('img').length > 0) {
      // If an image already exists, change its src 
      //$target2.find('img').attr('src', newSrc2);
      $target2.find('image-element').html(`<img class="photoswipe__image image-element" src="${newSrc2}">`);
    } else {
      // Else, append a new image
      $target2.html(`<img src="${newSrc2}" class="product-image">`);
    }
  ...
  $(document).on('mouseout', '.swatch_preview-img', function() {
    // Restore original content
    const $video2 = $target2.find('video');

    if ($video2.length) {
      const videoClone2 = $video2.clone(); // Clone the video element
      ...
      $target2.html(videoClone2); // Replace target with the video

      const videoEl2 = $target2.find('video').get(0);
      if (videoEl2) {
        videoEl2.play().catch(e => {
          console.warn("Autoplay failed:", e);
        });
      }
    } else {
    $target2.html(originalContent2);
      }
  });
  ...
  $('tool-tip-trigger[data-tool-tip="colorvariations-sidedrawer"]').click(function() {
    $('.product-single__sticky:last-child').addClass('additionalswatche__popup-opened');
  });

  $(document).on('click', '.tool-tip__close', function() {
    $('.product-single__sticky:last-child').removeClass('additionalswatche__popup-opened');
  });

  // Close popup on clicking outside the popup-content
  $(document).mouseup(function(e) {
    let popupContent = $('tool-tip[data-tool-tip="colorvariations-sidedrawer"] .tool-tip__inner');
...
/* pave text correction--normalize-pave-frontend.js*/
(function () {
  // Regex notes:
  // - We match word-boundary + 'pav' + (grave-accent e, e with combining grave, or plain 'e' not followed by combining acute)
  // - We intentionally DO NOT match 'pavé' (precomposed e-acute U+00E9) so correct forms stay unchanged.
  const COMBINING_GRAVE = "\u0300";
  const COMBINING_ACUTE = "\u0301";
  const rePavWrong = new RegExp(
    `\\b(pav(?:[èÈ]|e${COMBINING_GRAVE}|E${COMBINING_GRAVE}|e(?!${COMBINING_ACUTE})|E(?!${COMBINING_ACUTE})))\\b`,
    "giu"
  );

  function paveReplacer(match) {
    // Normalize combining marks for case checks: turn combining marks into plain 'e'
    const normalized = match.normalize("NFC").replace(/\u0300/g, "e").replace(/\u0301/g, "e");

    if (normalized === normalized.toUpperCase()) return "PAVÉ";
    if (normalized === normalized.toLowerCase()) return "pavé";
    if (normalized[0] && normalized[0] === normalized[0].toUpperCase()) return "Pavé";
    return "pavé";
  }

  function normalizePaveInText(text) {
    if (!text || typeof text !== "string") return text;
    if (!rePavWrong.test(text)) return text;
    return text.replace(rePavWrong, (m) => paveReplacer(m));
  }

  // Walk text nodes efficiently using TreeWalker
  function fixTextNodes(root = document.body) {
    if (!root) return 0;
    const walker = document.createTreeWalker(
      ...
      NodeFilter.SHOW_TEXT,
      {
        acceptNode(node) {
          ...
          if (/(SCRIPT|STYLE|NOSCRIPT|CODE|PRE)/i.test(parentTag)) return NodeFilter.FILTER_REJECT;
    ...
    while ((node = walker.nextNode())) {
      const newVal = normalizePaveInText(node.nodeValue);
  ...
  // Fix certain attributes (alt, title, placeholder, aria-label), excluding fields where users type
  function fixAttributes(root = document) {
    const ATTRS = ["alt", "title", "placeholder", "aria-label"];
    const selector = ATTRS.map(a => `[${a}]`).join(",");
    const nodes = Array.from(root.querySelectorAll(selector));
    let changed = 0;
    nodes.forEach(el => {
      // Skip if inside input/textarea or contenteditable regions (we don't want to change user's typed content)
      if (el.closest("input, textarea, [contenteditable='true'], [contenteditable='']")) return;
      ATTRS.forEach(attr => {
        if (!el.hasAttribute(attr)) return;
        const val = el.getAttribute(attr);
        const newVal = normalizePaveInText(val);
        if (newVal !== val) {
  ...
  // Fix document-level metadata: title and common meta tags
  function fixDocumentMeta() {
    let changed = 0;
    // document.title
    if (document.title) {
      const newTitle = normalizePaveInText(document.title);
    ...
    metaSelectors.forEach(sel => {
      ...
      if (!m) return;
      const attr = m.hasAttribute("content") ? "content" : (m.hasAttribute("value") ? "value" : null);
      if (!attr) return;
      const val = m.getAttribute(attr) || "";
      const newVal = normalizePaveInText(val);
      if (newVal !== val) {
        m.setAttribute(attr, newVal);
        changed++;
      }
    });
    return changed;
  ...
  // One-off run to fix existing DOM
  function runOnce() {
    let total = 0;
    total += fixTextNodes(document.body);
    total += fixAttributes(document);
    total += fixDocumentMeta();
    return total;
  }

  // Observe DOM mutations to fix dynamically-inserted content (e.g., cart drawer, theme sections)
  function startObserver() {
    const observer = new MutationObserver(mutations => {
      // For performance: aggregate changed roots and process each root once
      const roots = new Set();
      for (const m of mutations) {
        if (m.type === "childList") {
          ...
          // direct text node change — fix the single node
          const node = m.target;
          if (node.parentElement && !/(SCRIPT|STYLE|NOSCRIPT|CODE|PRE)/i.test(node.parentElement.tagName)) {
            const newVal = normalizePaveInText(node.nodeValue);
      ...
      roots.forEach(root => {
        try {
          if (root.nodeType === Node.ELEMENT_NODE) {
            fixTextNodes(root);
            fixAttributes(root);
            // also check if mutated node is <title> or meta (rare): call fixDocumentMeta just in case
            if (root.tagName === "TITLE" || root.tagName === "META") fixDocumentMeta();
          }
        } catch (e) {
          // swallow errors on dynamic nodes
          console.error("pave-normalizer observer error:", e);
        }
      });
    });
    ...
    // Return observer so it can be disconnected if desired
    return observer;
  }

  // Run after DOM ready (or immediately if already loaded)
  function init() {
    try {
      const fixed = runOnce();
      // console.log(`[pave-normalizer] initial fixes: ${fixed}`);
      startObserver();
    } catch (err) {
      console.error("pave-normalizer init error:", err);
    }
  }
...
(function () {
  ...
  const targetDate = new Date("2025-10-30T11:59:59").getTime();
  ...
  const timer = setInterval(() => {
      const now = new Date().getTime();
        ...
        // 🧠 Smart hiding logic
        if (days > 0) {
          display += `${days}:`; // no zero pad for days
        }
        if (hours > 0 || days > 0) {
          display += `${format(hours)}:`;
        }

        display += `${format(minutes)}:${format(seconds)}`;
  ...
  /**recent search history storage 
      (function () {
        var STORAGE_KEY = 'recent_searches_v1';
        var MIN_LEN = 2; // ignore too-short noise
        var MAX_STORED = 25; // cap storage size

        function nowISO() { return new Date().toISOString(); }
        function daysAgoISO(d){ const n = new Date(); n.setDate(n.getDate()-d); return n.toISOString(); }

        function loadStore(){
          try { return JSON.parse(localStorage.getItem(STORAGE_KEY)) || []; }
          catch(e){ return []; }
        }
        function saveStore(items){ localStorage.setItem(STORAGE_KEY, JSON.stringify(items)); }

        // Keep unique by case-insensitive normalized query
        function normalize(q){ return (q||'').trim().replace(/\s+/g,' ').toLowerCase(); }
        ...
        function addSearch(q){
          ...
          var items = loadStore();
          var key = normalize(q);
          var idx = items.findIndex(it => it.key === key);

          if (idx > -1) {
            // Update existing: bump timestamp and count, keep pretty version to latest casing
            items[idx].q = q;
            items[idx].lastAt = nowISO();
            items[idx].count = (items[idx].count || 1) + 1;
          } else {
            items.push({ q: q, key: key, firstAt: nowISO(), lastAt: nowISO(), count: 1 });
          }
        ...
        function purgeOlderThan(days){
          ...
          var items = loadStore().filter(it => it.lastAt >= cutoff);
          saveStore(items);
        }

        // Capture searches:
        // A) Query param on /search pages
        try {
        ...
        // B) Any search form submits (predictive or normal)
        document.addEventListener('submit', function(e){
          var form = e.target;
          if (!form || form.tagName !== 'FORM') return;
          var action = (form.getAttribute('action') || '').replace(location.origin, '');
          if (action === '/search' || action.startsWith('/search')) {
            var input = form.querySelector('input[name="q"]');
            if (input && input.value) addSearch(input.value);
          }
        }, true);
        ...
        function renderRecentLists(){
          ...
          var widgets = document.querySelectorAll('[data-recent-searches]');
          if (!widgets.length) return;

          widgets.forEach(function(wrap){
            var limit = parseInt(wrap.getAttribute('data-limit') || '5', 10);
            var days  = parseInt(wrap.getAttribute('data-days')  || '30', 10);
            ...
            var list  = wrap.querySelector('[data-recent-list]');
            var empty = wrap.querySelector('[data-recent-empty]');
            var clear = wrap.querySelector('[data-recent-clear]');
            ...
            if (!items.length) {
              ...
              items.forEach(function(it){
                ...
                li.className = 'recent-searches__item';
                var a = document.createElement('a');
                a.className = 'recent-searches__link';
                // preserve current search type/constraints if needed:
                // Build a URL with q only; customize as needed to add type=product, options, etc.
                var url = new URL('/search', window.location.origin);
                url.searchParams.set('q', it.q);
        ...
        // Re-render on load and when sections load (Shopify theme editor)
        document.addEventListener('DOMContentLoaded', renderRecentLists);
        document.addEventListener('shopify:section:load', renderRecentLists);
        document.addEventListener('shopify:section:reorder', renderRecentLists);
      })();
  */
function setPromoHeight(tries) {
  tries = tries || 0;
  if (tries > 20) return; // safety, don't loop forever

  // Take the first *visible* product link
  var $src = $('.grid-product__image-mask:visible').first();

  if (!$src.length) {
    setTimeout(function () { setPromoHeight(tries + 1); }, 300);
    return;
  }

  // Use outerHeight so padding is included
  var h = $src.outerHeight();
  
  if (h > 0) {
    $('.collection-promo--root[data-textpos="below_image"] .imagebnrr__promotion').height(h);
    $('.collection-promo--root[data-textpos="above_image"] .imagebnrr__promotion').height(h);
    $('.collection-promo--root[data-textpos="absoluted_onimage"] .imagebnrr__promotion').height(h);
  } else {
    setTimeout(function () { setPromoHeight(tries + 1); }, 300);
  }
}

function setPromo2Height(tries) {
  tries = tries || 0;
  if (tries > 20) return; // safety, don't loop forever

  // Take the first *visible* product link
  var $src = $('.productgridclass .grid-product__content:visible').first();
  var $src2 = $('.productgridclass.productgridclass__withswatches .grid-product__content:visible').first();

  if (!$src.length) {
    setTimeout(function () { setPromo2Height(tries + 1); }, 300);
    return;
  }
  ...
  // Use outerHeight so padding is included
  var h = $src.outerHeight();
  var h2 = $src2.innerHeight();
  
console.log('first_grid_height '+h);
console.log('withswatches_grid_height '+h2);

  if (h > 0) {
    $('.collection-promo--root[data-textpos="absoluted_onimage"]').height(h);
    $('.collection-promo--root[data-textpos="absoluted_onimage"] .imagebnrr__promotion').height(h);
  } else {
    setTimeout(function () { setPromo2Height(tries + 1); }, 300);
  }
  if (h2 > 0) {
    $('.collection-promo--root[data-textpos="absoluted_onimage"]').height(h2);
    $('.collection-promo--root[data-textpos="absoluted_onimage"] .imagebnrr__promotion').height(h2);
  } else {
    setTimeout(function () { setPromo2Height(tries + 1); }, 300);
  }
}
...
/*swatches +1more click function*/
function initVariantWrappers(context) {
  var $context = context ? $(context) : $(document);

  $context.find('.productgridclass .variant-wrapper').each(function () {
    var $wrapper = $(this);
    var $list = $wrapper.find('ul').first();

    // All product <li>, ignore any previous "+X" item
    var $items = $list.children('li').not('.js-variant-more');
    var visibleCount = 3;

    // Reset in case this runs again (Ajaxinate)
    $items.show();
    $list.find('.js-variant-more').remove();
    ...
    if (total > visibleCount) {
      ...
      // Create "+X" item
      //var $moreLi = $('<button type="button" class="variants-more">+' + hiddenCount + ' more</button>');

      // On click: show all and remove "+X"
...
// Init Slick for collection product carousels
function initCollectionSliders($scope) {
  var $root = $scope || $(document);

  $root.find('.js-product-slider').each(function () {
    var $slider = $(this);
    ...
    $slider.slick({
      ...
      speed: 300,
      adaptiveHeight: false,
      // optional: disable swipe on desktop if you like
      swipeToSlide: true,
      touchThreshold: 10,prevArrow: '<button type="button" class="slick-prev"> <svg width="10" height="20" viewBox="0 0 10 20" fill="none" xmlns="http://www.w3.org/2000/svg"> <path d="M9.75595 1.0774C10.0813 1.40278 10.0814 1.93047 9.75587 2.25589L2.01183 9.99979L9.75595 17.744C10.0813 18.0694 10.0814 18.5971 9.75587 18.9225C9.43042 19.248 8.9028 19.248 8.57739 18.9225L0.244062 10.589C0.0877991 10.4327 0 10.2208 0 9.99979C0 9.77878 0.0878353 9.56681 0.244099 9.41058L8.57742 1.07747C8.90284 0.751945 9.43049 0.751944 9.75595 1.0774Z" fill="#090909"/> </svg> </button>',
  nextArrow: '<button type="button" class="slick-next"> <svg width="10" height="20" viewBox="0 0 10 20" fill="none" xmlns="http://www.w3.org/2000/svg"> <path d="M0.244054 1.07752C-0.0813273 1.4029 -0.0813989 1.93059 0.244125 2.25601L7.98817 9.99991L0.244054 17.7442C-0.0813273 18.0695 -0.0813989 18.5972 0.244125 18.9227C0.569578 19.2481 1.0972 19.2481 1.42261 18.9227L9.75594 10.5891C9.9122 10.4329 10 10.2209 10 9.99991C10 9.77891 9.91216 9.56693 9.7559 9.4107L1.42258 1.07759C1.09716 0.752068 0.569506 0.752066 0.244054 1.07752Z" fill="black"/> </svg> </button>'        
    });

    // Prevent card-level click handlers from hijacking slider clicks
    $slider.on('click mousedown touchstart pointerdown', function (e) {
      e.stopPropagation();
    });
  });
}
...
$(document).ready(function () {
  initCollectionSliders($(document));
});
document.addEventListener('DOMContentLoaded', function () {
  var descriptionEl = document.querySelector('.collection-description');

  if (!descriptionEl) return;

  // 2. HOW TALL IS "COLLAPSED" STATE? (must match CSS max-height)
  var collapsedMaxHeight = 180; // in px — keep in sync with CSS

  // If content is already shorter than collapsed height, do nothing
  if (descriptionEl.scrollHeight <= collapsedMaxHeight + 5) {
    return;
  }

  // 3. START CLAMPED
  descriptionEl.classList.add('collection-description--clamped');

  // 4. CREATE THE TOGGLE BUTTON
  var toggleBtn = document.createElement('button');
  toggleBtn.type = 'button';
  toggleBtn.className = 'collection-description-toggle';
  ...
  toggleBtn.addEventListener('click', function () {
    ...
    if (expanded) {
      descriptionEl.classList.remove('collection-description--clamped');
  ...
  // 5. INSERT THE BUTTON AFTER THE DESCRIPTION (HTML stays untouched)
  descriptionEl.insertAdjacentElement('afterend', toggleBtn);
});

/****RebuyCart Drawer*** */
document.addEventListener('DOMContentLoaded', function() {
  // Function to transform variant titles
  function transformVariantTitles() {
    const variantTitles = document.querySelectorAll('.rebuy-cart__flyout-item-variant-title');
    
    variantTitles.forEach(function(element) {
      const variantText = element.textContent.trim();
      
      // Skip if already transformed or empty
      if (!variantText || variantText.includes(':')) return;
      
      // Split variant title by " / " to get individual options
      const options = variantText.split(' / ');
      
      // Common option names mapping
      const optionNames = ['Size', 'Color', 'Style', 'Material'];
      
      // Transform each option
      const transformedOptions = options.map(function(option, index) {
        const optionName = optionNames[index] || 'Option ' + (index + 1);
        return optionName + ': ' + option;
      });
      
      // Update the element
      element.textContent = transformedOptions.join(' / ');
    });
  }
  
  // Run initially
  transformVariantTitles();
  
  // Run when cart updates (using MutationObserver)
  const observer = new MutationObserver(transformVariantTitles);
...
$(document).on('click', '#CartDrawer .js-drawer-close', function () {
    $('#CartDrawer').removeClass('drawer--is-open');
    $('html').removeClass('js-drawer-open');
});


// document.addEventListener('drawerOpen.FilterDrawer', function () {
//   if (window.jQuery) {
//     // 🔑 Remove ONLY the click-to-close handler for filter drawer
//     jQuery(window).off('click.drawer-collectionFilters');
//   }
// });


// document.addEventListener('collection:reloaded', function () {
//   const countEl = document.querySelector('.collection-filter__item--count');
//   if (!countEl) return;

//   const count = countEl.textContent.match(/\d+/);
//   if (!count) return;

//   countEl.textContent = `${count[0]} Products.`;
// });
...
document.addEventListener('collection:reloaded', function () {
  ...
  const hasActiveFilters =
    document.querySelectorAll('.tag--active, .filter-tag--active, .collection-filter__active').length > 0;

  clearBtn.classList.toggle('hidden', !hasActiveFilters);
});


$('.custom_DrawerClose').click(function(e) {
  $('#CollectionSidebar').find('#FilterDrawer').removeClass('is-transitioning');
}); 
$('#FilterDrawer .drawer__scrollable .collection-filter__item--count').click(function(e) {
  $('#CollectionSidebar').find('#FilterDrawer').removeClass('is-transitioning');
  $('#CollectionSidebar').find('#FilterDrawer').removeClass('drawer--is-open');
...
$(document).ready(function() {
  ...
  var $subtitle = $(".product-single__title small");
  if (!$subtitle.length) return;

  var originalText = $subtitle.text();

  $(".multicolor__variations--available a")
    .on("mouseenter", function() {
      var hoverTitle = $(this).attr("title");
      if (hoverTitle) {
        $subtitle.text(hoverTitle);
      }
    })
